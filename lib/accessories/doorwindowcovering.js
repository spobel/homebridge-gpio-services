...
class MotorizedOpenClose extends require('../gpioAccessory.js').GPIOAccessory {
    ...
    initService() {
        ...
        new (require('../gpioCharacteristics.js')).GpioSwitch(this.log, this.engineStop, this.pin, this.invertHighLow, this.onChangeEngineStop.bind(this));

        this.engineUp = new Service.Switch(this.name + "engineOpen").getCharacteristic(Characteristic.On);
        new (require('../gpioCharacteristics.js')).GpioSwitch(this.log, this.engineUp, this.pinOpen, this.invertHighLowOpen, this.onChangeEngineOpen.bind(this));

        this.engineDown = new Service.Switch(this.name + "engineClose").getCharacteristic(Characteristic.On);
        new (require('../gpioCharacteristics.js')).GpioSwitch(this.log, this.engineDown, this.pinClose, this.invertHighLowClose, this.onChangeEngineClose.bind(this));
    ...
    onChangeEngineClose() {
        if (this.engineDown.value) {
            ...
            setTimeout(() => {
                this.hap.engineDown.updateValue(0);
            }, this.invokeTimeout);
        }
    }
    onChangeEngineStop() {
        if (this.engineStop.value) {
            this.log("trigger off in " + this.invokeTimeout + "ms.");
            setTimeout(() => {
                this.hap.engineStop.updateValue(0);
            }, this.invokeTimeout);
        }
    }
    ...
    onChangeEngineOpen() {
        if (this.engineUp.value) {
            this.log("trigger off in " + this.invokeTimeout + "ms.");
            setTimeout(() => {
                this.hap.engineUp.updateValue(0);
            }, this.invokeTimeout);
        }
    }
